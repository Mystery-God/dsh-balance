...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function BalanceFloat() {
      ...
      useEffect(() => {
        refresh(false)
        if (ctxInterval !== null) return ctxInterval(() => refresh(false), POLL_MS)
    ...
    /** Client ctx — captured at apply for the poll timer. */
    let ctxInterval = null

    function apply(ctx) {
      ctxInterval = typeof ctx.interval === 'function' ? ctx.interval.bind(ctx) : null
