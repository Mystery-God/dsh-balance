...
const DEFAULT_SETTINGS = { floatingWindow: true }
  ...
  async function fetchBalance(force) {
    ...
    inflight = (async () => {
      try {
        ...
        const hit = credentialsService !== undefined ? await credentialsService.resolve(refName) : undefined
        const key = (hit !== undefined && hit !== null && typeof hit.value === 'string' && hit.value.length > 0)
          ? hit.value
          : undefined
        if (key === undefined) {
          const data = { ok: false, error: '未找到 API key: ' + refName }
          cache = { at: now, data }
          return data
        }
